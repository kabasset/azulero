...
which is a rectangle from which video frames are extracted.
The image and viewport can be seen as analogous to a scene and camera.
...
`azul roam` is configured by a sequence file in YAML format, which lists key frame parameters.
For each key frame:
...
- The viewport size is specified either as a percentage relative to the image pixel size, or relative to the image extents.
...
Time is given with key `t` in seconds with suffix `s` or number of frames with suffix `f`.
...
Typically, with `a: 0pi` in a key frame and `a: 4pi` in the next one, the viewport would perform a full turn.
...
Several successive key frames can be eluded.
...
-- which will run continuously until the sixth key frame to reach 90° clockwise rotation.
...
In the next five seconds, we go back to the center of the image and dezoom to reach full image height again.
The viewport is finally fixed for one second.